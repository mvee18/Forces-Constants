...
# First derivatives.
def sub_first_derivative(input,name):
    molpro = Molpro(exevar="MOLPRO_EXE",
                runopts=["-s", "-n", "1"],
                scratch="/ddn/scratch1/r2561")
    return False

def calc_first_deriative(e,eneg):
    first_energy = (float(e2) - float(eneg))/(2*differential)
    print(first_energy)
    return
